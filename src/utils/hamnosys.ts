...
function convertToSigml(gloss: string, hamSymbols: string): string {
    ...
    for (const sys of hamSymbols.split(", ")) {
        const s = sys.trim();
        if (!s || !(s in _symToUnicode)) continue;
        sigmlText += `\t\t\t<${sys}/>\n`;
    }
...
function hamToUnicode(ham: string): string | null {
    if (!(ham in _symToUnicode)) return null;
    return _symToUnicode[ham];
}
function unicodeToHam(unicode: string): string | null {
    if (!(unicode in _unicodeToSym)) return null;
    return _unicodeToSym[unicode];
}
